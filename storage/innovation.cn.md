...
并发写入的正确性是不依赖选主的。即便完全不进行协调，自由去争抢写入event log也能通过乐观锁保证正确性。但是为了提高效率，最好是让写入都从一个node进行，以最大化地减少乐观锁冲突。从库也是一样，如果让同一个partition的数据更多地从几个node进行读取，可以提高缓存的命中效率。

所谓选主问题，其实是集群变化下的重新调整问题。可以分为两部分

* 集群变化的检测：节点全新启动，集群扩容，节点故障。归纳起来就是node的绝对数量的调整。
* 集群变化后的调整：从无master到有master，从无slave到有slave，从slave过多过少调整到刚好，从一个node担任过多的maste到刚好，从一个node担任过多的slave到刚好。归纳的起来就是给定n个node，m个partition，去对应两者之间的关系。
