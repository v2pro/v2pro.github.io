...
* docstore的集群并不单独进行选主的过程。初始的主从手工指定。当主挂了之后，其他的从在代理请求失败的之后，自动默认自己是主。只要event提交成功，就认为自己已经是主了，然后更新集群拓扑到kvstore里。在主从切换的瞬间，会有多个从同时尝试切为主，这个时候如果对一个entity有并发的写入，那么就会产生乐观锁冲突，会短暂下降写入性能。主再更新了之后，根据更新的时间戳，一段时间内不再重复切换，避免来回反复切。
