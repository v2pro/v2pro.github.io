...
大部分的主存储虽然可以支持一些查询能力，但是越来越多的时候，我们需要用各种视图以不同的形式重新组织数据，使得查询可以变得更快。除了更快之外，它也支持把多个数据源组合成一个统一的视图。使得查询的时候代码可以更简单。

在主存储负责业务逻辑一致性，负责并发控制，视图负责各种查询的背景下。现有的方案都没能很好地担当这个主存储的职责：

* MySQL 因为强制了schema，导致加新字段要重写所有已有的数据。这个导致了数据必须归档，否则热数据太多就搞不动了。其次加字段在各种trigger方案的补丁之下，仍然无法做到对业务完全无感知。
* 现有的文档db提供的能力容易引起有并发问题的误用。而且在对同一个热点文档有比较多的并发更新的时候，没有协调的乐观锁也无法达到比较好的性能。
* 视图的同步目前最常用的方案还是MySQL的binlog同步，然后通过binlog的有序消费来保证异构存储的更新。但是这样的解决方案的问题是及时性无法得到保证，在延迟敏感的场景下会有更高的概率出问题。其次整个基础设施还是比较复杂，牵涉到binlog的canal服务，中间的队列服务，以及一些消费程序。这就导致了，实际上业务用得更多的其实是业务层双写存储的，所谓简单方案。
* RPC 请求的幂等性在现有的数据库上是很难实现的。至少是很难高性能地实现的。因为现有的主流db都是只保存最终状态不去记录状态的变更的记录的，这样当一个操作过来重试的时候，我们其实都不知道这个操作之前做过了。

基于这样的背景，目标是构建一个文档数据库，做为数据的主存储实现以下三个目标：

* 简单好用
	* 方案简单可依赖：核心的可靠性只依赖于 MySQL
	* 无 Schema：读写的就是 JSON
	* 支持各种复杂的业务逻辑：对文档的更新直接写 javascript，多复杂都可以表达，不用学新的语言，也不受所谓query language的限制
* 并发问题全托管
	* 任何写入更新都原生支持幂等：开发者就不用自己去想其他办法来保障幂等了
	* 写代码的时候完全不用考虑并发更新的问题，由框架来处理冲突的情况
